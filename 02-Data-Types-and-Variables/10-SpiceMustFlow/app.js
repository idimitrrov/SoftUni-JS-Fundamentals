...
on the third day 10 less than on the second, and so on (see examples). A source is considered profitable only while its yield 
is at least 100 – when less than 100 spices are expected in a day, abandon the source. 
...
function mineSpices(startYield) {
  let dayCounter = 0;
  let spicesCounter = 0;

  while (startYield >= 100) {
    dayCounter++;
    spicesCounter += startYield;
